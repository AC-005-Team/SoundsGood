<template lang="html">
  <div class="mt-2 mb-12 mr-2 gap-4" @mouseover="mouseOver"  @mouseleave="mouseLeave" @click="playPause">
      <div class="parent h-48 w-48">
    ...
    <img class="object-cover h-48 w-48  overflow-hidden child" :src="this.top.audio.cover">
  </div>
  <i class="fab fa-gratipay"></i>{{ this.top.liked_users}}
  <div class="">{{ this.top.user.display_name }} </div>
  <div class="text-xs text-gray-500">{{ this.top.audio.name }}</div>
  </div>
...
export default {
  ...
  computed: {
    ...
    playing() {
      if (this.playerTracks.song_id === this.top.song_id ) {
  ...
  methods: {
    ...
    playPause() {
      if (this.playerTracks.song_id !== this.top.song_id || !this.playerTracks) {
        this.$store.dispatch('songs/pause')
        this.$store.dispatch('song/loadSong', this.top.song_id)
      } else {
        if (this.isPLAY === true) {
          this.$store.dispatch("songs/pause");
