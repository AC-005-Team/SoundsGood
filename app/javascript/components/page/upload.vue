<template lang="html">
  <div class="p-20 bg-cover h-screen">
    <form @submit.prevent="uploadForm">
      <div
        class="mt-2 m-auto w-2/3 border border-gray-400 p-4 bg-white shadow-2xl"
      >
        <div class="text-center">
          <p class="py-2 text-2xl">
            Upload Your Music, audio type must be mp3,ogg,mpeg
          </p>

          <ValidationProvider
            rules="required|ext:mp3,ogg,mpeg"
            ref="provider"
            name="audio"
      ...
      <div class="w-2/3 m-auto text-xs">
        <div
          class="flex justify-between items-center bg-gray-200 p-2 border border-gray-300 rounded"
        >
          <div class="flex"></div>
        </div>
          ...
          <div class="flex my-2">
            ...
            >
              <div class="w-full max-h-full rounded p-2 bg-gray-200 relative">
                ...
                Upload Image
                <ValidationProvider
                  rules="required|ext:jpg,png,jpeg"
                  ref="image_area"
                  name="image"
                  v-slot="{ errors, validate }"
                >
            ...
            <div class="w-2/3 mx-4 text-sm">
              ...
              >
                <input
                  class="mb-4 border border-gray-400 rounded w-full p-1"
                  v-model="name"
                  name="song[name]"
                  id="song_name"
                />
        ...
        >
          <button
            type="submit"
            class="submit text-2xl px-2 text-white bg-orange-600 rounded border"
            @click.prevent="uploadForm"
          >
            SUBMIT
          </button>
          <div class="text-sm">
...
export default {
  ...
  methods: {
    ...
    // },
    async showMyImage(e) {
      const { valid } = await this.$refs.image_area.validate(e);
...
$soundsgood: #f4441d;
$white: #fff;
$curve: cubic-bezier(0.85, 0, 0.55, 1.3);
$speed: 1.3s;
