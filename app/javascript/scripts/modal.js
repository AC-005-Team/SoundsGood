...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (modalBtn) {
    modalBtn.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        modalDiv.innerHTML = document.querySelector(
          `.${btnTemplate}`
        ).innerHTML;
        modalDiv.querySelector(".modal-container").innerHTML =
          '<div class="modal-fade"><div class="modal-mask"></div><button class="modal-close-btn"> X </button>';
  ...
  function getBtnTemplate(node, n) {
    return (
      node.classList[n].substring(0, node.classList[n].length - 4) + "-template"
    );
  }
});
